...
public class CartManager {
    ...
    public static void updateQuantityInCart(Context context, Produk produk, int newQuantity) {
        List<Produk> cart = getCart(context);
        boolean productFound = false;

        for (Produk p : cart) {
            if (p.getId_produk().equals(produk.getId_produk()) && p.getId_user().equals(produk.getId_user())) {
                if (newQuantity > 0) {
