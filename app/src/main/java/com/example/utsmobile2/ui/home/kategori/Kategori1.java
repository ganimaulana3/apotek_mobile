...
public class Kategori1 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        txtNoResult = findViewById(R.id.kategori1);
        rvKategori = findViewById(R.id.rvKategori1);
        rvKategori.setLayoutManager(new GridLayoutManager(Kategori1.this, 2));

        ApiService apiService = ApiClient.getClient().create(ApiService.class);
        apiService.getKategori1().enqueue(new Callback<List<Produk>>() {
            @Override
            public void onResponse(Call<List<Produk>> call, Response<List<Produk>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    produkList = response.body();
                    adapter = new ProdukAdapter(Kategori1.this, produkList, txtNoResult); // ← PERBAIKAN DI SINI
                    rvKategori.setAdapter(adapter);
                } else {
                    Toast.makeText(Kategori1.this, "Data kosong atau tidak sesuai", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<Produk>> call, Throwable t) {
                Toast.makeText(Kategori1.this, "Gagal terhubung ke server", Toast.LENGTH_SHORT).show(); // ← PERBAIKAN DI SINI
            }
        });
    }
}